window.onload = function () {
    ...
    if (! !!document.createElement('canvas').getContext) { //перевірка, чи є підтримка рисування
        document.getElementsByTagName('section')[0].innerHTML = '<p id="error">Ваш браузер не може відобразити плеєр</p><a href="index.html">Повернутися на головну</a>';
        return;
    }
    ...
    var string = 'Dance Dance Dance '; //бігаючий рядок плеєра
    source.width = 100;
    ...
    function running_string ( ) { //бігаючий рядок - постійне зміщення рядку на символ
        ...
        var text = string;
        if (text.length < 20) {
            for (var i = 0, len = 20 - text.length; i < len; i++) {
                text = text + ' ';
            }
        }
