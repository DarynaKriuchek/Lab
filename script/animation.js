...
function print_on_canvas(canv, contxt) { //друкуємо текст поверх анімації
    ...
    contxt.fillText('Ласкаво просимо', 10, canv.height - 11);
}
function draw_welcome(){
    "use strict";
    var welcome = document.getElementById('7Afd');
...
function winload() {
    ...
    window.onresize = function () { //масштабування анімації
        if (window.innerWidth >= 450 && window.innerWidth <= 600) {
            document.getElementById('7Afd').width = window.innerWidth - 30;
            document.getElementById('7Afd').height = 150;
        }
        else {document.getElementById('7Afd').height = 222;}
        print_on_canvas(document.getElementById('7Afd'), document.getElementById('7Afd').getContext('2d'));
    };
    //задання розмірів при запуску
    document.getElementById('7Afd').width = window.innerWidth >= 450 && window.innerWidth <= 600 ? window.innerWidth - 30 : 500;
    document.getElementById('7Afd').height = window.innerWidth >= 450 && window.innerWidth <= 600 ? 150 : 222;
    document.getElementById('7Afd').onclick = function() {
        if (cont) {
            cont = false;
        }
        else {
